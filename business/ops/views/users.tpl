            ...
            <div class="row">
                <div class="col-md-2">
                    {{if .user.Access "/group/add"}}
                        <a class="btn bg-purple btn-block margin-bottom" data-target="#add-group" data-toggle="modal"
                           title="添加">添加角色
                                            ...
                                            <li class="active">
                                                <a>{{$name}}
                                                    {{if $.user.Access "/group/{id:[0-9]+}"}}
                                                        <span class="btn btn-xs bg-navy pull-right"
                                                              data-href="/group/{{$id}}" data-target="#modal-node"
                                ...
                                <form class="form-inline">
                                    <div class="form-group">
                                        <div class="input-group input-group-sm">
                                            <input type="email" class="form-control" placeholder="请输入管理员邮箱" name="email"
                                                   value="{{.form.Get "email"}}"
                                                   required>
                                            <span class="input-group-btn">
                          <button type="submit" class="btn btn-info btn-sm" title="搜索"><i
                                      class="fa fa-search"></i></button>
                        </span>
                                        </div>
                                    </div>
                                    {{if .user.Access "/user/add"}}
                                        <a class="btn bg-purple btn-sm" data-target="#add-user" data-toggle="modal"
                                           title="添加">添加 <i class="fa fa-plus"></i></a>
                                    {{end}}
                                </form>
                            </div>
                        </div>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                {{if and ($.user.Access "/user/delete/{id:[0-9]+}") (ne .ID $.user.ID)}}
                                                    <a class="btn btn-default btn-xs" title="删除 {{.Email}}"
                                                       data-href="/user/delete/{{.ID}}"
                                                       data-target="#modal-confirm" data-toggle="modal"><i
