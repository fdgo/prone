...
func ExistRoleByID(id int) (bool, error) {
	var role Role
	err := db.Select("id").Where("id = ? AND deleted_at = ? ", id, 0).First(&role).Error
...
func GetRole(id int) (*Role, error) {
	var role Role
	err := db.Preload("Menu").Where("id = ? AND deleted_at = ? ", id, 0).First(&role).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return nil, err
	}
...
func CheckRoleName(name string) (bool, error) {
	var role Role
	err := db.Where("name = ? AND deleted_at = ? ", name, 0).First(&role).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return false, err
	}
	if role.ID > 0 {
...
func CheckRoleNameId(name string, id int) (bool, error) {
	var role Role
	err := db.Where("name = ? AND id != ? AND deleted_at = ? ", name, id, 0).First(&role).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return false, err
	}
	if role.ID > 0 {
...
func EditRole(id int, data map[string]interface{}) error {
	...
	if err := db.Where("id = ? AND deleted_at = ? ", id, 0).Find(&role).Error; err != nil {
		return err
	}
...
func CleanAllRole() error {
	if err := db.Unscoped().Where("deleted_at != ? ", 0).Delete(&Role{}).Error; err != nil {
		return err
	}
