...
func ExistMenuByID(id int) (bool, error) {
	var menu Menu
	err := db.Select("id").Where("id = ? AND deleted_at = ? ", id, 0).First(&menu).Error
...
func GetMenu(id int) (*Menu, error) {
	var menu Menu
	err := db.Where("id = ? AND deleted_at = ? ", id, 0).First(&menu).Error
	if err != nil && err != gorm.ErrRecordNotFound {
		return nil, err
	}
	return &menu, nil
...
func EditMenu(id int, data interface{}) error {
	if err := db.Model(&Menu{}).Where("id = ? AND deleted_at = ? ", id, 0).Updates(data).Error; err != nil {
		return err
	}
	return nil
}
...
func CleanAllMenu() error {
	if err := db.Unscoped().Where("deleted_at != ? ", 0).Delete(&Menu{}).Error; err != nil {
		return err
	}
	return nil
}
...
func EditMenuGetRoles(id int) []int {
	...
	db.Model(&menu).Where("id = ? AND deleted_at = ? ", id, 0)
